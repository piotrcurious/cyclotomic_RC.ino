...
void findResonance() {
  ...
  if (diff < 0 && prevDiff > 0) {
    ...
    // The formula is derived from https://www.electronics-tutorials.ws/rc/rc_1.html
    absolute_resistance = sqrt((pow(2, LFSR_BITS) - 1) / (CAPACITOR_VALUE * pow(resonance, 2) * log(2)));
...
void convergeLFSR() {
  // Define a variable to store the best LFSR state so far
  uint8_t bestLFSR = lfsr;
  ...
  for (uint8_t tap0 = 0; tap0 < LFSR_BITS; tap0++) {
    for (uint8_t tap1 = tap0 + 1; tap1 < LFSR_BITS; tap1++) {
      for (uint8_t tap2 = tap1 + 1; tap2 < LFSR_BITS; tap2++) {
        for (uint8_t tap3 = tap2 + 1; tap3 < LFSR_BITS; tap3++) {
          ...
          // Update the LFSR state and output pin with the new taps
          updateLFSR();
          
          // Read the RC circuit voltage and compute its resistance and frequency with the new LFSR state
          readRC();
          ...
          if (error < bestError) {
            // Update the best LFSR state and best error so far with the current values
            bestLFSR = lfsr;
            bestError = error;
          }
        }
  ...
  // Restore the global LFSR state with the best LFSR state found
  lfsr = bestLFSR;
  
  // Print a message to indicate that the LFSR has converged to the resonant frequency with a certain error
  Serial.print("LFSR has converged to ");
  Serial.print(resonance);
  Serial.print(" Hz with an error of ");
